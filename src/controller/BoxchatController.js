...
class BoxchatControl extends Controller {
  constructor(callback) {
    super(callback);
  }
}
export const BoxchatController = {
  getBoxchat: new BoxchatControl(async (req, res) => {
    const boxlist = await Boxchat.get();
    const list = boxlist.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    res.send(list);
  }).asyncHandler(),
  addBoxchat: new BoxchatControl(async (req, res) => {
