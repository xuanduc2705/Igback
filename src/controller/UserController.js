...
export const Register = new UserController(async (req, res) => {
...
export const getUser = new UserController(async (req, res) => {
  const userlist = await User.get();
  const list = userlist.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  res.send(list);
}).asyncHandler();
export const login = new UserController(async (req, res) => {
